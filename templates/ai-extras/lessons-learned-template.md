...
- **正确做法**：（应该怎么写/怎么做）
- **检测命令**（可选）：（grep/mvn/npm 命令可自动检测此问题；应返回空表示合规）
...
## LL-001 [{YYYY-MM-DD}] {第一条踩雷记录标题}

- **症状**：（描述用户/CI/测试观察到的现象，如接口报错、构建失败、测试不通过）
- **根因**：（定位到具体代码行为或设计决策，不能停留在"代码写错了"层面）
- **正确做法**：（给出具体的正确写法或操作步骤）
  ```
  // ❌ 错误示例（可选）
  ...
  // ✅ 正确示例（可选）
  ```
- **检测命令**：`# 替换为实际可运行的检测命令，应返回空表示合规`
- **触发的规则更新**：
  - 新增 `.ai/anti-patterns.md` AP-001（若可机器检测）
  - 更新 `.ai/skills/[相关技能]/skill.md`（若涉及编码规范）

---

> **提示**：此文件为新项目初始化模板。实际使用时，请将 `{占位符}` 替换为真实内容，
> 并删除此提示行。文件路径：`.ai/lessons-learned.md`
