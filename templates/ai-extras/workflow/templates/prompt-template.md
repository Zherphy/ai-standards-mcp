...
- **相关文件**：
  ...
  - `[相关测试文件路径]`
  - `[相关配置文件路径]`
- **相关技能**：`[SKILL_NAME_1]`、`[SKILL_NAME_2]`
...
- 遵循分层架构规则（Controller → Service → DAO）
- 使用构造函数注入依赖
- 异常统一使用 Result<T> 返回
- 不引入新的外部依赖（除非必要）
```
...
## [CONTEXT] 上下文
- 在 [模块名称] 模块中新增 [功能描述]
- 需修改文件：Controller / Service / DAO（按需）
- 需新建测试：对应的 Test 类

## [STEPS] 执行步骤
1. 编写 Service 层测试（Red）
2. 实现 Service 层逻辑（Green）
3. 编写 Controller 层测试（Red）
4. 实现 Controller 层端点（Green）
5. 重构优化（Refactor）
6. 更新修改记录
