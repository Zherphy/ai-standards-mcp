...
- 该错误模式**可以用命令自动检测**（grep、find、lint 规则等）
...
检测：`# 替换为实际检测命令，如：grep -rn "错误模式" src/main/ --include="*.java"`
（返回空 = 合规；有输出 = 违规）
...
> **提示**：此文件为新项目初始化模板。实际使用时，请将示例 AP-001 替换为真实反模式，
> 并删除此提示行。文件路径：`.ai/anti-patterns.md`
