...
> ⚠️ **强制要求**：前置条件必须在**本次会话中实际运行命令并看到通过输出后**才算满足。
> **禁止**用以下理由跳过：
> - "上次运行过" / "之前通过了"
> - "记忆中没有错误"
> - "功能很简单，不需要验证"
...
```bash
# 1. 测试全部通过（必须实际运行，确认输出无 FAIL）
[TEST_CMD]

# 2. 代码风格检查通过（必须实际运行，确认无新增 violation）
[LINT_CMD]

# 3. 覆盖率满足要求（≥ 80%，必须实际运行并看到数字）
[TEST_COVERAGE_CMD]
```
...
| 测试真实性 | 空断言 / 直通测试 / 无意义 Mock / **辅助函数零调用** / **调用真实外部服务** | `[Fail]` |
| 边界覆盖 | 关键入参无空值保护；空 catch 块；**WriteHeader 后设置响应头**；**void 函数内部写响应后调用方继续写入** | `[Warning]` / `[Fail]` |
